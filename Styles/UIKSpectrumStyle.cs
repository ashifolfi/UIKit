...
public class UIKSpectrumStyle : UIKitStyle
{
    ...
    public override string ThemeID => "adobe-spectrum";

    public override SKTypeface DefaultTypeface => m_SansFace ?? SKTypeface.Default;
    ...
    public override void DrawPrimitive(SKCanvas canvas, PrimitiveType primitiveType, StyleState state, Widget widget)
    {
        switch (primitiveType)
        {
            ...
            default:
                canvas.DrawRect(
                    new SKRect(0, 0, widget.Size.X, widget.Size.Y),
                    new SKPaint
                    {
                        Color = SKColors.Red,
                        IsStroke = true,
