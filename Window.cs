...
public class Window : IDisposable
{
    ...
    public bool HasBorder
    {
        get
        {
            unsafe
            {
                return (Sdl.GetApi().GetWindowFlags(m_NativeWindow) & (uint)WindowFlags.Borderless) == 0;
            }
        }
        ...
        set
        {
            unsafe
            {
                Sdl.GetApi().SetWindowBordered(m_NativeWindow, value ? SdlBool.True : SdlBool.False);
            }
        }
    }
    ...
    public Window(string title, Vector2D<int> size, Vector2D<int> pos)
    {
        ...
        unsafe
        {
            m_NativeWindow = m_Sdl.CreateWindow(
                title,
                pos.X < 0 ? Sdl.WindowposCentered : pos.X,
                pos.Y < 0 ? Sdl.WindowposCentered : pos.Y,
                size.X, size.Y,
    ...
    ~Window()
    {
        // todo: figure out why we have this deconstructor
        ReleaseUnmanagedResources();
    }
}
